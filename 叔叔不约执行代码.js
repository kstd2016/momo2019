...
function 全局变量(){
    ...
    联众账号 = "kstd2016";
    联众密码 = "Wentao1987223";
    微信 = Array("шēī✘","шēī♥","шēī❤","徽✘","徵♥","徾❤","幑♡","徴♡","шēī信")
    引流号 = Array("   ℡³6²¹º","   ТЕL³6²¹º","   ССЬЬ¹995º²")
    qq = 微信[random(0,微信.length-1)]+引流号[random(0,引流号.length-1)]
    //"   ССЬЬ¹995º²"
    ...
    套路回复[1] = Array(叔叔不约表情[random(0,叔叔不约表情.length-1)]+"你好啊.小哥哥",叔叔不约表情[random(0,叔叔不约表情.length-1)]+"你好啊"
    ,叔叔不约表情[random(0,叔叔不约表情.length-1)]+"哈喽")
    套路回复[2] = Array(叔叔不约表情[random(0,叔叔不约表情.length-1)]+"小哥哥多大啊",叔叔不约表情[random(0,叔叔不约表情.length-1)]+"你多大呀"
    ,叔叔不约表情[random(0,叔叔不约表情.length-1)]+"你多少岁呀",叔叔不约表情[random(0,叔叔不约表情.length-1)]+"你几岁呀")
    套路回复[3] = Array(叔叔不约表情[random(0,叔叔不约表情.length-1)]+"有点想,污吗",叔叔不约表情[random(0,叔叔不约表情.length-1)]+微信[random(0,微信.length-1)]+"直接污吗,有点想",叔叔不约表情[random(0,叔叔不约表情.length-1)]+"坐标杭州,想找个厉害的 有意加"+微信[random(0,微信.length-1)]
    ,叔叔不约表情[random(0,叔叔不约表情.length-1)]+"有点想,"+微信[random(0,微信.length-1)]+"污吗",叔叔不约表情[random(0,叔叔不约表情.length-1)]+微信[random(0,微信.length-1)]+"连麦爱爱吗",叔叔不约表情[random(0,叔叔不约表情.length-1)]+"老公不在家好无聊 你能陪我聊聊吗")
    套路回复[4] = Array("那"+微信[random(0,微信.length-1)]+"污?方便点", "那"+微信[random(0,微信.length-1)]+"说?,方便些", "要不"+微信[random(0,微信.length-1)]+"说?,方便些",微信[random(0,微信.length-1)]+"语音?")
    套路回复[5] = Array(qq,qq)
    //套路回复[1] = "你好，小哥哥| 你好呀，小哥哥|小哥哥好".split("|")
    //套路回复[2] = "连麦玩吗？有点想|玩连麦爱爱吗? 有点想|连麦爱爱吗,我想试试".split("|")
    //套路回复[3] = "要么∨聊吧|薇聊吗,方便点|∨聊吧,方便点|要不薇聊吧".split("|")
...
function 进入叔叔不约(){
    ...
    for(let a=0;a<20;a++){
        ...
        if(超级判断("立即提速")||超级判断("立即安装")||超级判断("立即下载")){
            集成模块.找图点击(叔叔不约X图标保存路径,800,500,200,300,相似度)
        }
    }
    sleep(1000)
    封装API.text("允许").click();
    ...
    for(let a=0;a<10;a++){
        ...
        if(集成模块.找图点击(叔叔不约离开保存路径,0,1570,320,200,相似度)=="找图成功"){
            ...
            log(获取号码)
            sleep(500)
            if(className("EditText").text("手机号").findOne(1000)==null){
                log("没有找到手机号文本控件")
                className("EditText").clickable(true).boundsInside(435,993,855,1125).setText(获取号码)
            }
            else{text("手机号").setText(获取号码)}//手机号码
            sleep(500)
            封装API.text("点击获取").click();
            封装API.desc("点击获取").click();
            sleep(500)
            if(className("EditText").text("密码(至少需要6位)").findOne(1000)==null){
                log("没有找到密码(至少需要6位)文本控件")
                className("EditText").clickable(true).boundsInside(315,1284,1035,1416).setText("abcd1995")
            }
            else{text("密码(至少需要6位)").setText("abcd1995")}//密码
            sleep(500)
            break;
        }
    ...
    if(text("反馈问题").exists()==true){
        ...
        if(获取号码参数==1){
            ...
            if(获取验证码=="获取验证码失败"){
                ...
                if(className("EditText").text("手机验证码").findOne(1000)==null){
                    log("没有找到手机验证码文本控件")
                    className("EditText").clickable(true).boundsInside(315,137,555,1269).setText(验证码)
                }
                else{text("手机验证码").setText(验证码)}//验证码
                超级点击("确认")
            }
        }
    }
...
function 开始聊天(){
    ...
    for(let b=1;b<套路回复.length;b++){
        ...
        else if(text("窗口").exists()==true&&text("菜单").exists()==true){
            toastLog("找到窗口还有菜单,判断在浏览器主页...")
            进入叔叔不约();
            break;
        }
        else if(descContains("请稍等").exists()==true||textContains("请稍等").exists()==true){
            sleep(30*1000)
...
function 固定时间回复(变量){
    for(let a=0;a<回复等待时长;a++){
        ...
        if(回复消息 == "无关键词回复"&&时间变量==回复等待时长-1){
                setText(套路回复[变量][random(0,套路回复[变量].length-1)])
                if(变量==0){套路回复深度.push(0)}
                封装API.等待(500,1000)
                超级点击("发送")
                超级点击("留步")
                超级点击("分享")
        ...
        else  if(回复消息 == "无关键词回复"&&时间变量!=回复等待时长-1){}
        else if(回复消息=="没有获取到消息"){}
        else{
            时间变量 = 0
            log(回复消息)
            setText(回复消息)
...
function 关键词回复模块(){
    ...
    关键词[1] = Array("聊污","聊骚","聊色","文","语爱","聊色")
    关键词[2] = Array("视频", "连麦", "语音")
    ...
    关键词[10] = Array("哪里人","哪里的","哪儿的","哪儿人","什么地方的","什么地方人")
    ...
    关键词[17] = Array("约过吗", "做过吗","约过没","做过没")
    关键词[18] = Array("多大","几岁","多少岁")
    关键词[19] = Array("很近","好近","有点近")
    关键词[20] = Array("鸡鸡","硬","鸡巴")
    关键词[21] = Array("不会说话")
    关键词[22] = Array("多高")
    关键词[23] = Array("怎么啦")
    关键词[24] = Array("在吗")
    ...
    关键词[28] = Array("在么","在吗")
    关键词[29] = Array("怎么了")
    随机回复[0] = Array("你好呀", "你好啊,小哥哥", "你好","嗨喽")
    随机回复[1] = Array("好啊,那"+微信[random(0,微信.length-1)]+"污?方便点", "可以啊,那"+微信[random(0,微信.length-1)]+"说麽,方便些")
    随机回复[2] = Array("好啊，那我们"+微信[random(0,微信.length-1)]+"聊吧", "恩,那"+微信[random(0,微信.length-1)]+"聊吧方便点", "恩,可以啊,那"+微信[random(0,微信.length-1)]+"聊吧")
    随机回复[3] = Array("我在看电视", "我在看抖音呀", "我在刷抖音")
    随机回复[4] = Array("那我们"+微信[random(0,微信.length-1)]+"聊吧方便点，"+"!", "加我"+微信[random(0,微信.length-1)]+"聊吧方便点，"+"!", "那加我"+微信[random(0,微信.length-1)]+"聊吧方便点，"+"!")
    随机回复[5] = Array("我很少用那个，我都是用微信的"+"!", "我不用QQ的 要不加个V吧 "+"!", "没有用只有微信"+"!")
    随机回复[6] = Array("在呢,这个用的的比较少，要不加我微信聊吧"+"!","在呢 怎么了","加我"+微信[random(0,微信.length-1)]+"聊吧"+"!"+"探探用得少")
    随机回复[7] = Array("我这里上的少 你加我"+微信[random(0,微信.length-1)]+"聊吧"+"!"+"","我探探用的不太多啦，要不你加我微信把"+"!")
    随机回复[8] = Array("没有呢，你呢", "暂时没有呢，你呢", "目前没有，你呢")
    随机回复[9] = Array("恩，"+"!", "可以,"+"!", "恩,那我留个"+微信[random(0,微信.length-1)]+"吧，"+"!")
    ...
    随机回复[17] = Array("恩,玩过呀,你呢")
    随机回复[18] = Array("我23,你呢")
    随机回复[19] = Array("确实蛮近的", "是挺近的", "好像确实蛮近的[花痴]")
    随机回复[20] = Array("要不要帮你呀,"+微信[random(0,微信.length-1)]+"一块污,嘻嘻")
    ...
    随机回复[25] = Array("很少,我一般都是啪啪啪,嘻嘻")
    随机回复[26] = Array("同乐同乐","谢谢,","谢谢,同乐")
    随机回复[27] = Array("恩,有点想呢,шēī✘连麦吗")
    随机回复[28] = Array("在啊","在呀")
    ...
    for(let a=0;a<关键词.length;a++){
        ...
        关键词[a].forEach(element => {
            if(消息.indexOf(element) != -1 ){
                ...
                if(a==1||a==2||a==4||a==20||a==27){引流微信参数 = 1;}
            } 
        });
    } 
    if(关键词回复变量 == 1){
                    ...
                    if(滑块!=null){
                        ...
                        if(坐标 != "打码失败"){
                            ...
                            if(滑块按钮坐标X1>="100"&&滑块按钮坐标X1<="150"){
                                封装API.randomSwipe(滑块按钮坐标X1+10,滑块按钮坐标Y1+10,拼图坐标X+random(15,25),滑块按钮坐标Y1+10+random(0,10));
                            }
                            else{
                                log("没有找到滑块按钮坐标,启用备用坐标...")
                                封装API.randomSwipe(136,1279,拼图坐标X+random(15,25),1279+random(0,10))
                            }
                        }
                        else{
