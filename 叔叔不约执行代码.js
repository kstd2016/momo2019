...
function 全局变量(){
    ...
    套路回复[1] = Array(叔叔不约表情[random(0,叔叔不约表情.length-1)]+"玩连麦爱爱吗? 有点想，我шēī✘,",叔叔不约表情[random(0,叔叔不约表情.length-1)]+"有互看的吗,我шēī✘,",叔叔不约表情[random(0,叔叔不约表情.length-1)]+"有相互看的吗,我шēī✘,",叔叔不约表情[random(0,叔叔不约表情.length-1)]+"老公不在家我一个人 你能陪我语音吗 我шēī✘")
    //套路回复[1] = "你好，小哥哥| 你好呀，小哥哥|小哥哥好".split("|")
    //套路回复[2] = "连麦玩吗？有点想|玩连麦爱爱吗? 有点想|连麦爱爱吗,我想试试".split("|")
    //套路回复[3] = "要么∨聊吧|薇聊吗,方便点|∨聊吧,方便点|要不薇聊吧".split("|")
...
function 进入叔叔不约(){
    ...
    for(let a=0;a<20;a++){
        ...
        if(text("立即提速").exists()||text("立即安装").exists()){
            集成模块.找图点击(叔叔不约X图标保存路径,800,600,200,200)
        }
    }
    sleep(1000)
    封装API.text("允许").click();
